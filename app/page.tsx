...
const DECK = [
 "Breakfast for dinner","A window seat","Fresh flowers","A long nap","Dancing in the kitchen","A surprise day off","Homemade cookies","A beach sunset","Winning an argument","A perfectly ripe mango","A road trip","A cozy blanket","Karaoke night","A handwritten note","The first sip of coffee","A clean house","Getting dressed up","A funny family story","A quiet morning","A spontaneous plan","A great bargain","Staying up too late","A home-cooked meal","A mystery novel","A warm rainstorm","Board game night","A new restaurant","An old photograph","No alarm tomorrow","A thoughtful gift","Being right","A long phone call","Freshly washed sheets","A movie marathon","A beautiful garden","A big family dinner","Trying something new","A second helping","A scenic train ride","A silly inside joke","A perfect playlist","Finishing a puzzle","An unexpected compliment","A lazy Sunday","A fancy dessert","A museum afternoon","A good hair day","A heartfelt hug",
 "Retiring somewhere warm","Building a dream home","Starting a family business","Learning a new language","Taking a family world tour","Creating a new holiday tradition","Living abroad for a year","Finally doing the biggest bucket-list dream","Hosting every future family gathering","Having a five-year plan that actually works",
...
export default function Home(){
 ...
 const next=()=>{if(turn+1>=total){setPhase("finished");return}setTurn(turn+1);setStep(0);setPicks([]);setRanking([]);setPhase("pass")};
 const reset=()=>{setPhase("setup");setTurn(0);setPicks([]);setRanking([])};
 const high=Math.max(...scores),winners=players.filter((_,i)=>scores[i]===high);
 return <main className="app-shell">
  <header className="brand-row"><button className="brand" onClick={reset}><span className="brand-mark">?</span><span>Goofy Doofy M3</span></button>{phase!=="setup"&&phase!=="finished"&&<div className="round-pill">Turn {turn+1} of {total}</div>}</header>
  ...
  <footer>Made for Monica, Makarand & Mira <span>♥</span></footer>
 </main>
}
